    ...
    for k in range(noSensors):
        ...
        for j in range(noDataTypes):
            ...
            for i_l in range(noPlots):
                ...
                sc = plt.scatter(QoI_obs, QoI_sim,c=dfSim_i['meandir'],edgecolors='black',s=None, cmap=twilight,label=mastNames[i] +": z = "+ str(z) +" " + "Corr. "+"{0:.2f}".format(corr_uu[0]))
                plt.colorbar(sc,label="Observation"+" "+"wind"+" "+"direction"+" "+"($^{\circ}$)")
                plt.plot([0, 25], [0, 25],'k-',linewidth=4)
                ...
                plt.legend()
                plt.tight_layout()
                plt.savefig(simraResultsFolder+'scatterPlots/'+"Mean_wind"+"_"+location+"_"+str(z)+"_"+dataTypes[j]+"_%d%02d.pdf"  % (year,month))
                plt.close()
                #plt.show()
